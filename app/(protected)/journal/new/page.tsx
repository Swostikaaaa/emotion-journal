      ...
      <div className="max-w-3xl w-full glass-card p-8">
        ...
        {detectedEmotion && (
          <div className="bg-white/20 border border-white/30 text-white p-4 rounded-xl mb-6 text-center space-y-1">
            <p className="text-lg font-semibold">✨ Detected emotion: <span className="capitalize">{detectedEmotion}</span></p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <p className="text-xs text-white/50 mt-2">✨ Your emotion will be detected automatically when you save.</p>
          </div>
          <div className="flex justify-end gap-3">
            ...
            <button type="submit" disabled={submitting} className="inline-flex items-center gap-2 px-6 py-2 rounded-full bg-white/20 hover:bg-white/30 text-white font-semibold transition disabled:opacity-50"><Save size={18} /> {submitting ? 'Analysing your feelings...' : 'Save Entry'}</button>
          </div>
        </form>
      </div>
    </main>
